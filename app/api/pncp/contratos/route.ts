...
export async function GET(request: NextRequest) {
  ...
  try {
    const url = `${PNCP_BASE}/v1/contratos/publicacao?${params.toString()}`;
    ...
    if (!response.ok) {
      ...
      console.error("[v0] PNCP contratos error:", response.status, errorText);
    ...
    console.error("[v0] Contratos proxy error:", error);
    return NextResponse.json({ data: [], totalRegistros: 0 }, { status: 200 });
  }
}
